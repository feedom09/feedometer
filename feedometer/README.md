...
After deploy, note your Worker URL (e.g. `https://feedom.<account>.workers.dev`).

This project’s live API is `https://feedom.feedom-8f1.workers.dev`. Keep these in sync:

- `scripts/feedometer-config.js` → `PRODUCTION_API`
- `wrangler.toml` → `[vars] WORKER_PUBLIC_URL`
- Centralized Frontend API → `scripts/feedometer-config.js`
...
(currently `https://feedometer-4vg.pages.dev`), so they always open the active Pages UI rather
